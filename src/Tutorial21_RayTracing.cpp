...
void Tutorial21_RayTracing::CreateRayTracingPSO()
{
    ...
    RefCntAutoPtr<IShader> pCubePrimaryHit, pGroundHit, pGlassPrimaryHit, pSpherePrimaryHit, pSphereSolidHit, pSphereVidrioHit;
    {
        ...
        ShaderCI.Desc.Name  = "Sphere solid primary ray closest hit shader";
        ShaderCI.FilePath   = "SphereSolidHit.rchit";
        ShaderCI.EntryPoint = "main";
        m_pDevice->CreateShader(ShaderCI, &pSphereSolidHit);
        VERIFY_EXPR(pSphereSolidHit != nullptr);
        
        ShaderCI.Desc.Name  = "Sphere solid primary ray closest hit shader";
        ShaderCI.FilePath   = "SphereVidrioHit.rchit";
        ShaderCI.EntryPoint = "main";
        m_pDevice->CreateShader(ShaderCI, &pSphereVidrioHit);
        VERIFY_EXPR(pSphereVidrioHit != nullptr);
    }
    ...
    PSOCreateInfo.AddProceduralHitShader("SphereVidrio", pSphereIntersection, pSphereVidrioHit);

    PSOCreateInfo.AddProceduralHitShader("SphereSolidHit", pSphereIntersection, pSphereSolidHit);
...
void Tutorial21_RayTracing::UpdateTLAS()
{
    ...
    if (smallSpherePositions.empty())
    {
        ...
        int         perRing   = MaxSmallSpheres / rings;
        ...
        for (int ring = 1; ring <= rings; ++ring)
        {
            ...
            {
                float ang = 2 * 3.1416f * (j / float(perRing));
                float x   = R * std::cos(ang);
                float z   = R * std::sin(ang);

                float jitter = (rand() % 100) / 100.0f * 2.0f - 1.0f;
                x += jitter;
                z += jitter;
        ...
        int remaining = MaxSmallSpheres - smallSpherePositions.size();
    ...
    for (int i = 0; i < MaxSmallSpheres; ++i)
    {
        ...
        inst.Transform.SetTranslation(pos.x, -5.34f, pos.z);
    }
...
void Tutorial21_RayTracing::CreateSBT()
{
    ...
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance", PRIMARY_RAY_INDEX, "SphereVidrio");
    m_pSBT->BindHitGroupForInstance(m_pTLAS, "Sphere Instance 2", PRIMARY_RAY_INDEX, "SphereSolidHit");
    ...
    for (int i = 0; i < NumSmallSpheres; ++i)
        {
            ...
            const char* hitGroupName = (i < NumSmallSpheres / 1.5) ? "SphereSolidHit" : "SpherePrimaryHit"; 
    ...
    const char* sphereNames[] = {
        "Sphere Instance",
        "Sphere Instance 2",
        "Sphere Instance 3"};

    for (const char* name : sphereNames)
    {
        m_pSBT->BindHitGroupForInstance(
            m_pTLAS,
            name,
    ...
    instanceNames.reserve(NumSmallSpheres);
    for (int i = 0; i < NumSmallSpheres; ++i)
    {
        instanceNames.push_back("SmallSphere" + std::to_string(i));
    }
...
void Tutorial21_RayTracing::UpdateUI()
{
    ...
    SliderDef sliders[] = {
        {"Shadow blur", &m_Constants.ShadowPCF, 0, 16},
        {"Max recursion", &m_Constants.MaxRecursion, 0, static_cast<int>(m_MaxRecursionDepth)},
        {"Reflection blur", &m_Constants.SphereReflectionBlur, 1, 16}};

    for (auto& s : sliders)
        ImGui::SliderInt(s.label, s.value, s.min, s.max);

    // 4) ColorEdit para la esfera
